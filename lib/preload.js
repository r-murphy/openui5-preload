...
const processContentJs = (content) => (`jQuery.sap.registerPreloadedModules(${content});`)
...
function preload(options = {}) {
  ...
  function generatePreloadFile(preloadOption, preloadInfo, preloadFile) {
    ...
    const processedContent = preloadInfo.processContent(JSON.stringify(preloadObject, null, '\t'))
    ...
    if (preloadOriginalSize && preloadCompressedSize && preloadOriginalSize !== preloadCompressedSize) {
      let mm = maxmin({ length: preloadOriginalSize }, { length: preloadCompressedSize })
      log += ` (${mm})`
    }
  ...
  function minify(fileContent, fileName, options) {
    ...
    switch (fileExtension) {
      case '.js':
        const result = uglify.minify(fileContent, {
