...
const preloadTypeMap = {
  ...
  components: {
    ...
    processContent: (content) => (`jQuery.sap.registerPreloadedModules(${content});`),
...
function preload(options) {
  ...
  const allPreloadTypes = Object.keys(preloadTypeMap)
  ...
  for (const preloadType of preloadTypes) {
    const preloadInfo = preloadTypeMap[preloadType]
  ...
  function generatePreloadFile(preloadOption, preloadInfo, preloadFile) {
    ...
    // To track total filesize for logging
